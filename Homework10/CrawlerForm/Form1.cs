...
namespace CrawlerForm {
  public partial class Form1 : Form {
    ...
    private void Crawler_CrawlerStopped(Crawler obj) {
      Action action = () => lblInfo.Text = "爬虫已停止"+crawler.stopWatch.ElapsedMilliseconds;
    ...
    private void btnStart_Click(object sender, EventArgs e) {
            //清空已有的数据，相当于queue.clear()
            crawler.pending = new System.Collections.Concurrent.ConcurrentQueue<string>();
            resultBindingSource.Clear();
            crawler.StartURL = txtUrl.Text;
            Match match = Regex.Match(crawler.StartURL, Crawler.urlParseRegex);if (match.Length == 0) return;
